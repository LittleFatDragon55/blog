<template>
  <div class="tagcloud-all"
       ref="tagcloudall" style="margin-top: 143px;">
<!--          <span v-for="item in tagList" :key="item.id" :style="'color:' + item.color + ';top: 0;left: 0;filter:none;'"><router-link :to="'/Article/'+item._id">{{item.name}}</router-link></span>-->

    <a v-for="item in tagList" @click="href_click(item._id)"
...
export default {
  ...
  methods: {
    ...
    onReady() {
      this.$nextTick(() => {
        // console.log(this.$refs.tagcloudall,"this.$refs.tagcloudall")
    ...
    href_click(id){
      this.$router.push({name:'Article',params: {tag_id:id}})
    }
  },
  created() {
    this.$nextTick(() => {
